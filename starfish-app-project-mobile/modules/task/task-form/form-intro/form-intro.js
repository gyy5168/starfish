...
var View = Backbone.View.extend({
    ...
    render: function() {
        ...
        this.$el.html(__inline("form-intro.html"));
    ...
    set: function(intro) {
        var html = intro.replace(/\_\\n/g, "\r\n");
        this.$infoTextarea.val(html);
    },
    ...
    finishModify: function() {
        ...
        if (this.type == "create") {
            ...
            $.ajax({
                ...
                type: "PATCH",
                data: JSON.stringify({
                    content: that.$modifyTextarea.val()
                }),
                success: function(response) {
                    if (response.errcode === 0) {
                        that.$infoTextarea.val(that.$modifyTextarea.val());
                        Autosize.update(this.$infoTextarea);
                        that.trigger("change", {
                            content: that.$modifyTextarea.val()
                        })
