...
var View = Backbone.View.extend({
    ...
    initialize: function(opts) {
        this.departId = opts.id;
        this.render();
        this.seachDepartMembers(opts.id);
    },

    render: function() {
        this.$loading = $(".people-select-all").find(".JS-loading");
        this.$error = $(".people-select-all").find(".JS-error");
        this.$empty = $(".people-select-all").find(".JS-empty");
    ...
    seachDepartMembers: function() {
        var that = this;
        this.$error.hide();
        that.$empty.hide();
        this.$loading.show();
        $.ajax({
            url: global.data.org.get("domain") + "/orgs/" + global.data.org.id + "/departments/" + that.departId + "/items?page=1&count=30",
            type: "GET",
    ...
    seachMoreMembers: function(pageNum) {
        ...
        // that.$loading.hide();
        $.ajax({
            url: global.data.org.get("domain") + "/orgs/" + global.data.org.id + "/departments/" + that.departId + "/items?page=" + pageNum + "&count=30",
            type: "GET",
            success: function(response) {
                if (response.errcode === 0) {
                    if (response.data.length == 0) {
    ...
    addItemList: function(data) {
        ...
        $.each(data, function(index, item) {
            ...
            if (that.isSelected(item.item.id)) {
