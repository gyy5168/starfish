...
var View = Backbone.View.extend({
    ...
    render: function() {
        this.$el.html(__inline("search-people.html"))
    ...
    search: function(value) {
        ...
        that.showLoading();
        var url = global.baseUrl + "/orgs/" + global.data.currentOrg.get("id") +
            "/search?q=" + value + "&is_detail=1&type=101&page=1&count=" + that.pageSize;
        this.ajaxObj = $.ajax({
            url: url,
            type: "GET",
                ...
                if (response.errcode === 0) {
                    ...
                    if (data.length) {
                        _.each(data, function(model) {
                            if (model.source.is_left == 0) {
                                that.list.push(model.source)
                            }
                        });
                        that.showList();
    ...
    loadMore: function() {
        ...
        this.$moreError.hide();
        var url = global.baseUrl + "/orgs/" + global.data.currentOrg.get("id") +
            "/search?q=" + this.keyWord + "&is_detail=1&type=101&page=" + that.pageCount + "&count=" + that.pageSize;
        return $.ajax({
            url: url,
            type: "GET",
            success: function(response) {
