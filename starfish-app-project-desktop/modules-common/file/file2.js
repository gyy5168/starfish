...
window.starfishFile = function(response) {
	var callbacks;
	response = response.replace(/\'/g, "\"");
	response = JSON.parse(response);

	response.transferType = response.transferType || response.type;

	if ( response.transferType === "upload" ) {
		callbacks = uploadCallbacks[response.data.uuid];
	} else {
		callbacks = downloadCallbacks[response.data.uuid];
	}
	...
	if( response.state === "progress" ){
		handle(callbacks.progress, response.data);
		return;
	}

	if (response.state === "success" ) {
		handle(callbacks.success, response.data);
		handle(callbacks.complete, response.data);
		callbacks = null;
		return;
	}

	if (response.state === "error" ) {
		handle(callbacks.error, response.data);
		handle(callbacks.complete, response.data);
		// callbacks = null;
		return;
	}
