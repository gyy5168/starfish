...
class Command(BaseCommand):
    SQL = {
        'create_time':
        """ALTER TABLE %s DROP COLUMN create_time;""",
        'update_time':
            """ALTER TABLE %s DROP COLUMN update_time;""",
        'is_deleted':
        """ALTER TABLE %s DROP COLUMN is_deleted;"""
    }
        ...
        for m in get_models():
            ...
            if skip:
                ...
                if db_main and t in default_tables_dict.keys():
                    hits_default_dict[t] = \
                        [s for s in self.SQL.keys() if s in default_tables_dict[t]]
                if db_org and t in org_tables_dict.keys():
                    hits_org_dict[t] = \
                        [s for s in self.SQL.keys() if s in org_tables_dict[t]]
